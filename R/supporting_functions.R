...
# and label_col, respectively). The function first checks if the number of rows
...
# exceeds the number of colors availabel in the palette, the function increments
...
plot_dbscan <- function(mod_data, mod, id_col = 1, cluster_col = 2, label_col = 3){
  ...
  if(ncol(mod) > 3){
    warning(paste0("The input data frame has more than three columns. Only the first three columns will be used for analysis. The expected column names are 'ID' for column ", id_col, ", 'cluster' for column ", cluster_col, ", and 'label' for column ", label_col,". Please make sure your data is properly formatted."))
    mod <- mod[, 1:3]
  }
  
  names(mod)[id_col] <- "ID"
  names(mod)[cluster_col] <- "cluster"
  names(mod)[label_col] <- "label"
  
  warning(paste0("Using columns in training dataset in the following order: 'ID' for column ", id_col,
                 ", 'cluster' for column ", cluster_col, ", and 'label' for column ", label_col,
  ...
  df$label <- ifelse(df$label == 1, "core", "border")
  df$label <- ifelse(df$cluster == 0, "noise",  df$label)
  ...
  # If the number of clusters exceeds the maximum number of colors availabel in the palette,
  ...
  p <- ggplot(data = df, aes(x = x, y = y, color = cluster, shape = label)) +
    geom_point(size = 3)+
